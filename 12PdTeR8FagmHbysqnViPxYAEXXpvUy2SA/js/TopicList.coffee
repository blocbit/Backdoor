class TopicList extends Class
	...
	applyTopicData: (elem, topic, type="list") ->
		...
		$(".site .site-link", elem).attr({'href':site})
		$(".git .git-link", elem).attr({'href':git})
		$(".video .video-link", elem).attr({'href':video})
		...
		if User.my_topic_votes[topic_uri] # Voted on topic
			...
			#$(".active", elem).on "change", -> $(".score").not("active").addClass('voted')
		...
		if topic.topic_creator_address == Page.site_info.auth_address
			...
			# owners cannot vote for own concept
			$(".score", elem).addClass('owned');
			
																		#junk
																		#$(".score", elem).remove();
																		#$(".info", elem).append('<a class="scorevo"></a>');
																		#$(".scorevo", elem).text topic.votes
																		#$(".info", elem).addClass("scorev")
																		#if elem.hasClass("scorev")
