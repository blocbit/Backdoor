class BlocChat extends ZeroFrame
    ...
    sendMessage: =>
        ...
        inner_path = "data/users/#{@site_info.auth_address}/public.json"  # This is our data file
    ...
    loadMessages: (mode="normal") ->
        ...
        @cmd "dbQuery", [query], (messages) =>
            ...
            for message in messages
                ...
                message_lines.push "<li><small title='#{added}'>#{Time.since(message.date_added/1000)}</small> <b style='color: #{Text.toColor(message.cert_user_id)}'>#{message.cert_user_id.replace('@bloc.bit', '')}</b>: #{body}</li>"
            #if mode != "nolimit"
